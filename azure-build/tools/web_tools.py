...
from tools.tool_backend_helpers import prefers_gateway  # noqa: F401
...
def _get_backend() -> str:
    ...
    if configured:
        ...
        # Legacy managed "nous" selection → firecrawl BYOK path.
        if configured == "nous":
            return "firecrawl"
        return configured
...
def _web_requires_env() -> list[str]:
    ...
    set.  The behavioral contract is: if the env var
    is set, the tool sees it; if not, it doesn't.
    """
    return [
        "EXA_API_KEY",
        "PARALLEL_API_KEY",
