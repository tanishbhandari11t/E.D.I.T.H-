...
# Managed Nous tool gateway removed — BFL FLUX3 tools are unavailable without it.

logger = logging.getLogger(__name__)


def managed_vendor_endpoints(_vendor: str):
    return None


def managed_gateway_auth_headers(_url: str):
    return {}


def build_managed_media_uploader(_base_url: str, _upload_path: str):
    return None
...
_SIGN_IN_MESSAGE = (
    "BFL video generation via the managed Nous Tool Gateway has been removed. "
    "This toolset is unavailable until a direct BFL/BYOK path is added."
)
...
def check_bfl_requirements() -> bool:
    """Managed Nous gateway removed — BFL FLUX3 toolset stays hidden."""
    return False
