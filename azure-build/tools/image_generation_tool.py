...
from tools.fal_common import (
    _ManagedFalSyncClient,  # noqa: F401 — kept for tests that patch the name
...
# ---------------------------------------------------------------------------
# FAL credentials (direct / BYOK only — managed gateway removed)
# ---------------------------------------------------------------------------
def _resolve_managed_fal_gateway():
    """Managed FAL gateway removed — return None for direct FAL_KEY path.

    Raises ValueError when a stored selection lacks FAL_KEY.
    """
...
def _get_managed_fal_client(managed_gateway):
    """Unused after managed gateway removal; kept for call-site stability."""
    del managed_gateway
    raise RuntimeError("Managed FAL gateway has been removed")
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    _resolve_managed_fal_gateway()  # raises if selected path lacks FAL_KEY
...
def check_fal_api_key() -> bool:
    """True if FAL_KEY is configured for the selected (or default) FAL path."""
    selected = read_selection("image_gen")
    if selected is not None:
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", "nous"):
        # Unset/explicit FAL keeps the legacy FAL path; legacy "nous"
        # (removed managed gateway) also falls through to FAL BYOK.
        return None
...
def _maybe_route_managed_krea(
    ...
    """Managed Krea gateway removed — always fall through to the normal pipeline."""
    del prompt, aspect_ratio, image_url, reference_image_urls, upscale
    return None
