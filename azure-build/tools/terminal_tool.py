    ...
    if env_type == "local":
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["mode"] == "managed":
                raise ValueError(
                    "Modal backend is configured for managed mode, but the "
                    "managed Nous Tool Gateway has been removed. Set "
                    "TERMINAL_MODAL_MODE=direct/auto and configure direct "
                    "Modal credentials."
                )
    ...
    try:
        ...
        if env_type == "local":
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["mode"] == "managed":
                    logger.error(
                        "Modal backend selected with TERMINAL_MODAL_MODE=managed, but the "
                        "managed Nous Tool Gateway has been removed. Configure direct Modal "
                        "credentials and set TERMINAL_MODAL_MODE=direct/auto."
                    )
                    return False
