...
def _resolve_azure_foundry_runtime(
    *,
    ...
    AZURE_OPENAI_* env vars are aliases for the Foundry vars:
    ``AZURE_OPENAI_API_KEY`` → ``AZURE_FOUNDRY_API_KEY``,
    ``AZURE_OPENAI_ENDPOINT`` → ``AZURE_FOUNDRY_BASE_URL``,
    ``AZURE_OPENAI_API_VERSION`` appends ``?api-version=`` when missing,
    ``AZURE_OPENAI_DEPLOYMENT`` fills an empty model/deployment name.
    ...
    # reasoning models as Responses-API-only on some resources.  Calling
    # /chat/completions against them can return 400.  Upgrade api_mode when
    # the model name matches — but only when the user did NOT explicitly set
    # model.api_mode (so chat_completions can be forced for deployments that
    # still speak Chat Completions, e.g. gpt-5.6-sol on this resource).
    # AZURE_OPENAI_DEPLOYMENT is an alias when no model/deployment is set.
    explicit_api_mode = (
    ...
    if not effective_model:
        effective_model = _getenv("AZURE_OPENAI_DEPLOYMENT", "").strip()
    if (
        effective_model
    ...
    # AZURE_OPENAI_ENDPOINT is an alias for AZURE_FOUNDRY_BASE_URL.
    env_base_url = (
        _getenv("AZURE_FOUNDRY_BASE_URL", "").strip()
        or _getenv("AZURE_OPENAI_ENDPOINT", "").strip()
    ).rstrip("/")
    base_url = explicit_base_url_clean or cfg_base_url or env_base_url
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'hermes model' or "
            "the AZURE_FOUNDRY_BASE_URL (or AZURE_OPENAI_ENDPOINT) environment variable."
        )

    # Bare ``https://{resource}.openai.azure.com`` 404s on /chat/completions.
    # OpenAI-compatible traffic needs ``/openai/v1`` (Foundry docs) or the
    # classic ``/openai/deployments/{name}`` path. Prefer /openai/v1.
    try:
        _parsed_azure = urlparse(base_url)
        _host = (_parsed_azure.hostname or "").lower()
        _path = (_parsed_azure.path or "").rstrip("/")
        if _host.endswith(".openai.azure.com") and _path in ("", "/"):
            base_url = urlunparse(
                _parsed_azure._replace(path="/openai/v1", query=_parsed_azure.query)
            )
    except Exception:
        pass
    ...
    # AZURE_OPENAI_API_VERSION — append ?api-version= when the URL lacks it.
    # For /openai/v1 endpoints the supported value is typically ``v1`` (classic
    # YYYY-MM-DD versions return "API version not supported").
    api_version = _getenv("AZURE_OPENAI_API_VERSION", "").strip()
    _path_lower = (urlparse(base_url).path or "").lower()
    if "/openai/v1" in _path_lower and (
        not api_version or re.match(r"^\d{4}-\d{2}-\d{2}", api_version)
    ):
        api_version = "v1"
    if api_version and "api-version=" not in base_url.lower():
        sep = "&" if "?" in base_url else "?"
        base_url = f"{base_url}{sep}api-version={api_version}"
    ...
    # AZURE_OPENAI_API_KEY is an alias for AZURE_FOUNDRY_API_KEY.
    api_key = explicit_api_key
    if not api_key:
        try:
            from hermes_cli.config import get_env_value
            api_key = (
                get_env_value("AZURE_FOUNDRY_API_KEY")
                or get_env_value("AZURE_OPENAI_API_KEY")
                or ""
            )
        except Exception:
            api_key = ""
    if not api_key:
        api_key = (
            _getenv("AZURE_FOUNDRY_API_KEY", "").strip()
            or _getenv("AZURE_OPENAI_API_KEY", "").strip()
        )
    if not api_key:
        raise AuthError(
            "Azure Foundry requires an API key. Set AZURE_FOUNDRY_API_KEY "
            "(or AZURE_OPENAI_API_KEY) in ~/.hermes/.env or run 'hermes model' "
            "to configure. To use keyless Microsoft Entra ID auth instead, set "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "nous":
        raise AuthError(
            "Nous Portal provider was removed from this build.",
            provider="nous",
