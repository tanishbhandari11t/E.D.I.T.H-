...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "azure-foundry": ProviderConfig(
        ...
        # AZURE_OPENAI_* are aliases for the Foundry vars (see runtime_provider).
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.code == "subscription_required":
        if error.provider == "nous":
            return f"{error} Nous Portal provider was removed from this build."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    """Legacy helper — Portal entitlement messaging was removed."""
    return f"{error} Nous Portal provider was removed from this build."
...
def resolve_provider(
    ...
    *,
    ...
    # 3. Azure Foundry (incl. AZURE_OPENAI_* aliases)  4. OPENAI/OPENROUTER
    # env keys  5. OpenRouter pool  6. provider-specific env keys
    # 7. auth.json `active_provider` (OAuth; never nous)  8. Bedrock  9. error.
    ...
    # Azure Foundry BEFORE OpenRouter — AZURE_OPENAI_* are aliases for Foundry.
    _azure_key = (
        has_usable_secret(_scoped_key_env("AZURE_FOUNDRY_API_KEY"))
        or has_usable_secret(_scoped_key_env("AZURE_OPENAI_API_KEY"))
    )
    if _azure_key:
        _azure_base = (
            (_scoped_key_env("AZURE_FOUNDRY_BASE_URL") or "").strip()
            or (_scoped_key_env("AZURE_OPENAI_ENDPOINT") or "").strip()
        )
        if not _azure_base and isinstance(_model_cfg, dict):
            _cfg_base = str(_model_cfg.get("base_url") or "").strip()
            _cfg_prov = str(_model_cfg.get("provider") or "").strip().lower()
            if _cfg_base and (
                _cfg_prov in {"azure-foundry", "azure", "azure-openai", ""}
                or "azure" in _cfg_base.lower()
                or "openai.azure.com" in _cfg_base.lower()
                or "services.ai.azure.com" in _cfg_base.lower()
            ):
                _azure_base = _cfg_base
        if _azure_base:
    ...
    try:
        ...
        # Skip removed Nous Portal — never auto-select it from a stale auth.json.
        if (
            _maybe
            and _maybe != "nous"
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "nous":
        return {
            "logged_in": False,
            "provider": "nous",
            "error": "Nous Portal provider was removed from this build.",
        }
