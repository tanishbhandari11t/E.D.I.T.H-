...
async def gated_auth_middleware(
    ...
    if bearer:
        ...
        if bearer_session is not None:
            ...
            bind_err = _enforce_bound_profile(request, bearer_session)
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            bind_err = _enforce_bound_profile(request, new_session)
            if bind_err is not None:
                return bind_err
    ...
    bind_err = _enforce_bound_profile(request, session)
    if bind_err is not None:
        return bind_err
    response = await call_next(request)
...
def _enforce_bound_profile(request: Request, session) -> Response | None:
    """Lock requests to the profile claimed by the auth session.

    When ``session.profile`` is non-empty (multi-user basic auth):
      * reject ``?profile=`` for any other profile with 403
      * force ``?profile=<bound>`` into the ASGI query string so every
        FastAPI handler and PTY/WS upgrade is scoped correctly even if
        the client omits the param
    """
    from urllib.parse import parse_qsl, urlencode

    bound = (getattr(session, "profile", None) or "").strip()
    if not bound:
        return None

    request.state.bound_profile = bound
    requested = (request.query_params.get("profile") or "").strip()
    if (
        requested
        and requested.lower() != "current"
        and requested != bound
    ):
        return JSONResponse(
            {"detail": "profile access denied"},
            status_code=403,
        )
    ...
    qs["profile"] = bound
    request.scope["query_string"] = urlencode(qs).encode("latin-1")
    return None
