...
def _nous_extra_body() -> dict:
    """Nous Portal tagging removed — return empty extra_body."""
    return {}
...
def _get_provider_chain() -> List[tuple]:
    ...
    """
    return [
        ("azure-foundry", _try_azure_foundry),
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Nous Portal was removed — never report paid entitlement."""
    return False
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            ...
            return True
        if normalized == "nous":
            # Nous Portal provider was removed — cannot refresh credentials.
            return False
            from hermes_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_auto_route(
    ...
    Priority:
      ...
      2. Azure Foundry → OpenRouter → custom → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_nous, _stale_base_url_warned
    auxiliary_is_nous = False  # Reset — Nous Portal is no longer in the chain
    runtime = _normalize_main_runtime(main_runtime)
...
def resolve_provider_client(
    ...
    if provider == "nous":
        logger.warning(
            "resolve_provider_client: Nous Portal provider was removed from "
            "this build — configure Azure Foundry, OpenRouter, or another provider."
        )
        return None, None
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "nous":
            logger.warning(
                "resolve_provider_client: Nous Portal provider was removed "
                "from this build"
            )
            return None, None
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "azure-foundry":
        return _try_azure_foundry(model=model)
    if provider == "nous":
        logger.warning(
            "Vision backend: Nous Portal provider was removed from this build"
        )
        return None, None
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → Azure Foundry → OpenRouter → stop.  This is the
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. Azure Foundry / OpenRouter — skip if already covered by main provider.
