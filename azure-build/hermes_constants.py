...
def _get_platform_default_hermes_home() -> Path:
    ...
    New installs prefer the Graphy project directory. Existing Hermes
    installs are dual-read: if the new path does not exist yet but the
    legacy hermes path does, keep using the legacy path so data is not lost.
    """
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        new_home = base / "graphy"
        old_home = base / "hermes"
    else:
        new_home = Path.home() / ".graphy"
...
def _hermes_home_from_env() -> Path:
    """Resolve home from the process environment only.

    Reads ``GRAPHY_HOME`` then ``HERMES_HOME``, falling back to the
    platform-native default (with dual-read of legacy hermes paths).
    ...
    """
    for key in ("GRAPHY_HOME", "HERMES_HOME"):
...
def get_hermes_home() -> Path:
    ...
    if not (
        os.environ.get("GRAPHY_HOME", "").strip()
