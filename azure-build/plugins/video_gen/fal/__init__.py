...
def _resolve_managed_fal_video_gateway():
    ...
    Raises ValueError when a stored non-FAL selection lacks FAL_KEY.
    """
...
def _get_managed_fal_video_client(managed_gateway):
    ...
    del managed_gateway
    raise RuntimeError("Managed FAL video gateway has been removed")


def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit a FAL video request using direct FAL_KEY credentials."""
    _load_fal_client()
    request_headers = {"x-idempotency-key": str(uuid.uuid4())}
    _resolve_managed_fal_video_gateway()  # raises if selected path lacks FAL_KEY
    return _fal_client.submit(endpoint, arguments=arguments, headers=request_headers)


def _check_fal_video_available() -> bool:
    """True if FAL_KEY is configured for the FAL video backend."""
    from tools.tool_backend_helpers import fal_key_is_configured, read_selection
