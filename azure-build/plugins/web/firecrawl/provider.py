...
def _get_firecrawl_gateway_url() -> str:
    """Managed Firecrawl gateway removed — always empty."""
    return ""


def _is_tool_gateway_ready() -> bool:
    """Managed tool gateway removed — always False."""
    return False
...
def _firecrawl_backend_help_suffix() -> str:
    """Optional help suffix for Firecrawl configuration errors."""
    return ""
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    raise ValueError(
        ...
        "The managed Nous Tool Gateway has been removed — use a direct API key."
    )


def _get_firecrawl_client() -> Any:
    """Get or create the cached Firecrawl client (direct / keyless only).

    Strict selection semantics (switch on the stored ``web`` selection):
    - any stored web backend → direct Firecrawl ONLY (or keyless when
      explicitly selected with no credentials);
    - never-configured web section → direct config when present, else error.
...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        """Return True when direct Firecrawl (or keyless) path is configured."""
