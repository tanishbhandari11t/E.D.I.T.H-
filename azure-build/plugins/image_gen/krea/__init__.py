...
def _resolve_managed_krea_gateway():
    """Managed Krea gateway removed — always None (direct/BYOK only)."""
    return None


def _managed_krea_gateway_ready() -> bool:
    """Managed Krea gateway removed — always False."""
    return False
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Krea",
            "badge": "paid",
            "tag": "Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Requires KREA_API_KEY.",
            "env_vars": [
                {
                    "key": "KREA_API_KEY",
                    "prompt": "Krea API key",
