...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        del refresh_token  # formerly gated managed-gateway token refresh
        ...
        # Managed Nous gateway removed — direct API key only.
        # A legacy "nous" selection is treated as unconfigured by read_selection.
        if api_key:
            return {
                "api_key": api_key,
                "base_url": _BASE_URL,
    ...
    def _get_config(self) -> Dict[str, Any]:
        ...
        if config is None:
            ...
            raise ValueError(
                ...
                "The managed Nous Tool Gateway has been removed."
            )
        return config
