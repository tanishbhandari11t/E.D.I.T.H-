...
def _serialize_billing_error(exc) -> dict:
    """Map a billing error into the result.error envelope the TUI branches on.

    Nous Portal billing modules were removed — always return a clear
    provider_removed envelope (no import of deleted modules).
    """
    return {
        "ok": False,
        ...
        "message": (
            str(exc)
            if exc
            else "Nous Portal billing was removed from this build."
        ),
        "portal_url": None,
        "retry_after": None,
        "payload": {},
...
def _serialize_billing_state(state) -> dict:
    ...
    def format_money(value):
        return None if value is None else str(value)  # Portal billing removed
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Jimmy" if role == "assistant" else role.title()
