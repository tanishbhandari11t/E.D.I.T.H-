...
# Legacy config may still store provider="nous" from the removed Tool Gateway.
NOUS_MANAGED_PROVIDER = "nous"

# Stubs replacing deleted hermes_cli.nous_subscription — features are always inactive.
MANAGED_FEATURE_COVERAGE_CATEGORY: dict = {}


class _InactiveFeature:
    available = False
...
class NousSubscriptionFeatures:
    """Compatibility shim: Nous subscription / Tool Gateway was removed."""
...
def get_nous_subscription_features(config=None, *, force_fresh: bool = False) -> NousSubscriptionFeatures:
    """Always inactive — Nous Subscription / Tool Gateway was removed."""
    return NousSubscriptionFeatures()


def apply_nous_managed_defaults(*_args, **_kwargs) -> bool:
    """No-op: managed Nous defaults are no longer applied."""
    return False


def format_nous_portal_entitlement_message(*_args, **_kwargs) -> str:
    return ""
...
def _visible_providers(
    ...
    *activates* the gateway once paid access is confirmed.
    """
    # Nous Subscription / Tool Gateway rows are gone; skip any leftover
    # managed_nous_feature / requires_nous_auth entries if present.
    visible = []
...
def _has_agent_browser() -> bool:
    """True when the agent-browser CLI is discoverable on PATH / local node."""
    import shutil
    ...
    try:
        ...
        for candidate in (
            ...
            PROJECT_ROOT / "node_modules" / ".bin" / "agent-browser",
...
def _local_browser_runnable() -> bool:
    """True when local browser automation (CLI + Chromium/Lightpanda) is ready."""
    ...
    try:
        ...
        # If browser_tool is unavailable, treat CLI presence as enough for
        # readiness probes (post_setup will still install Chromium).
        return True
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    if use_gateway:
        # Managed Nous gateway removed — keep the vendor name.
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed Nous gateway removed — do not write provider="nous".
        pass
...
def _configure_provider(
    ...
    *,
    ...
    # Managed Nous Tool Gateway rows were removed.
    if managed_feature:
        _print_warning(
            "  Nous Subscription / Tool Gateway was removed — pick a direct "
            "(BYOK) provider instead."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = provider.get("browser_provider") or "browser-use"
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ EDITH Tool Configuration", Colors.CYAN, Colors.BOLD))
